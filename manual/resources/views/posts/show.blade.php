        ...
        <main>
            <div class="main">
                <div class="post">
                    ...
                    <p class="post-info">投稿者: {{ $post->user->name }}</p>
                    <p class="post-info">カテゴリー: {{ $category->name }}</p>
                    <p class="post-info">メーカー: {{ $maker->name }}</p>
                    ...
                    @if(Auth::user()->bookmarks->contains($post->id))
                        <form action="{{ route('bookmarks.destroy', $post->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="bookmarked-button bookmarked">
                                ブックマーク解除
                            </button>
                        </form>
                    @else
